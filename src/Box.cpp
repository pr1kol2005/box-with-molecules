...
Vector Box::GetNormal(const Vector &pos) const {
  if (pos.x_ <= width_ / 2 && pos.x_ <= pos.y_ &&
      pos.x_ <= (height_ - pos.y_)) {
    return LEFT_WALL_NORMAL;
  } else if (pos.x_ >= width_ / 2 && (width_ - pos.x_) <= pos.y_ &&
             (width_ - pos.x_) <= (height_ - pos.y_)) {
    return RIGHT_WALL_NORMAL;
  } else if (pos.y_ <= height_ / 2 && pos.y_ <= pos.x_ &&
             pos.y_ <= (width_ - pos.x_)) {
    return BOTTOM_WALL_NORMAL;
  } else if (pos.y_ >= height_ / 2 && (height_ - pos.y_) <= pos.x_ &&
             (height_ - pos.y_) <= (width_ - pos.x_)) {
