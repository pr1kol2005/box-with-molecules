...
ParticleImage::ParticleImage(Particle *molecule) : molecule_(molecule) {
  ...
  QColor color =
      QColor((int)(255 * heat_value), 255, (int)(255 * (1 - heat_value)));
  setBrush(color);

  setRect(0, 0, molecule_->radius_ * 2, molecule_->radius_ * 2);

  setPos(molecule_->position_.x_ - molecule_->radius_,
         molecule_->position_.y_ - molecule_->radius_);
}
...
void ParticleImage::advance(int phase) {
  ...
  if (phase) {
    ...
    QColor color =
        QColor((int)(255 * heat_value), 0, (int)(255 * (1 - heat_value)));
    setBrush(color);
  }
}